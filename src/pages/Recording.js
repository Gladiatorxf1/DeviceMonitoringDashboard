...
function Recordings() {
  ...
  const fetchRecordings = async () => {
    ...
    const res = await fetch(
      `http://localhost:5000/api/recordings/${deviceId}?date=${date}`
    );
    ...
    <div className="p-4">
      ...
      <div className="flex flex-wrap justify-between items-center gap-2 mb-4">
        ...
        <div className="space-x-2">
          ...
          <button
            onClick={async () => {
              setIsSyncing(true); // Show syncing...
              await fetchRecordings(); // Call the same fetch
              setTimeout(() => {
                setIsSyncing(false); // Hide syncing message after 1s
                alert("🔄 Sync complete!");
              }, 1000);
            }}
                ...
                .map((rec) => (
                  <tr key={rec.id} className="hover:bg-gray-100">
                    ...
                    <td className="p-2 border">
                      {playingId === rec.id ? (
                        <audio
                          src={`http://localhost:5000/recordings/${rec.filename}`}
                          controls
                          autoPlay
